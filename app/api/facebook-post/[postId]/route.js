...
export async function GET(req, { params }) {
  ...
  try {
    console.log(`Making Facebook Graph API call for post ${postId} with account ${account}`);
    console.log(`Access token exists: ${!!accessToken}`);
    
    // Build the Facebook Graph API URL for post attachments
    const url = `https://graph.facebook.com/v19.0/${postId}?access_token=${accessToken}&fields=attachments`;
    console.log(`Facebook Graph API URL: ${url.replace(accessToken, '***')}`);
    
    const { data } = await axios.get(url);
    console.log(`Facebook Graph API call successful for post ${postId}!`);
    ...
    if (error.response?.data?.error) {
      return new Response(
        JSON.stringify({ 
          error: `Facebook API error for ${accountType} post ${postId}: ${error.response.data.error.message}` 
        }),
    ...
    return new Response(
      JSON.stringify({ 
        error: `Failed to fetch post data for ${postId} (${accountType}): ${error.message}` 
      }),
      { status: 500 }
    );
  }
